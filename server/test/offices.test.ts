...
async function createTestOffice(name = 'משרד לדוגמה') {
  const res = await request(app).post('/api/offices').send({ name });
...
describe('offices API', () => {
  it('creates an office and retrieves its full profile', async () => {
    ...
    const res = await request(app).get(`/api/offices/${office.id}`);
  ...
  it('returns 404 for an unknown office', async () => {
    const res = await request(app).get('/api/offices/does-not-exist');
  ...
  it('manages practice areas including sub-areas', async () => {
    const office = await createTestOffice();
    const parent = await request(app).post(`/api/offices/${office.id}/practice-areas`).send({ name: 'דיני משפחה' });
    ...
    const list = await request(app).get(`/api/offices/${office.id}/practice-areas`);
    expect(list.body).toHaveLength(2);

    const del = await request(app).delete(`/api/offices/${office.id}/practice-areas/${child.body.id}`);
    expect(del.status).toBe(204);
  });
  ...
  it('replaces the full weekly business-hours schedule', async () => {
    ...
    const res = await request(app).put(`/api/offices/${office.id}/business-hours`).send({ hours });
    expect(res.status).toBe(200);
  ...
  it('manages holidays', async () => {
    ...
    const list = await request(app).get(`/api/offices/${office.id}/holidays`);
    expect(list.body).toHaveLength(1);
    const del = await request(app).delete(`/api/offices/${office.id}/holidays/${add.body.id}`);
    expect(del.status).toBe(204);
  });
  ...
  it('manages handoff rules', async () => {
    ...
    const list = await request(app).get(`/api/offices/${office.id}/handoff-rules`);
    expect(list.body).toHaveLength(1);

    const del = await request(app).delete(`/api/offices/${office.id}/handoff-rules/${add.body.id}`);
    expect(del.status).toBe(204);
  });
