...
function uniqueEmail() {
  return `conv-owner-${Math.random().toString(36).slice(2)}@example.com`;
}

async function createOfficeWithSettings() {
  const agent = request.agent(app);
  const reg = await agent.post('/api/auth/register').send({ officeName: 'משרד בדיקה', name: 'הבעלים', email: uniqueEmail(), password: 'password123' });
  const office = { agent, ...reg.body.office };
  await agent.put(`/api/offices/${office.id}/disclosure`).send({ enabled: true, messageText: 'שיחה זו מנוהלת אוטומטית.' });
  await agent.put(`/api/offices/${office.id}/representative`).send({ name: 'נועה', role: 'digital_assistant' });
  await agent.put(`/api/offices/${office.id}/style`).send({ tone: 'professional' });
  await agent
...
describe('conversations API', () => {
  it('creates a conversation and starts it in auto status', async () => {
    ...
    const res = await office.agent.post(`/api/offices/${office.id}/conversations`).send({ contactPhone: '+972501234567' });
  ...
  it('auto-replies to the first inbound message with disclosure + representative intro', async () => {
    ...
    const res = await office.agent.post(`/api/offices/${office.id}/conversations/${conv.id}/inbound`).send({ text: 'שלום, יש לי שאלה' });
    ...
    const detail = await office.agent.get(`/api/offices/${office.id}/conversations/${conv.id}`);
  ...
  it('switches to pending_human and stops auto-replying once a handoff rule matches', async () => {
    ...
    const first = await office.agent.post(`/api/offices/${office.id}/conversations/${conv.id}/inbound`).send({ text: 'זה מקרה דחוף מאוד' });
    ...
    const second = await office.agent.post(`/api/offices/${office.id}/conversations/${conv.id}/inbound`).send({ text: 'עוד פרטים' });
  ...
  it('records a manual staff outbound message', async () => {
    const office = await createOfficeWithSettings();
    const conv = await office.agent.post(`/api/offices/${office.id}/conversations`).send({ contactPhone: '+972501234567' }).then((r) => r.body);
    const res = await office.agent.post(`/api/offices/${office.id}/conversations/${conv.id}/outbound`).send({ text: 'שלום, כאן עו״ד ישראלי' });
    expect(res.status).toBe(201);
  ...
  it('detects the practice area mentioned in the message and tags the conversation', async () => {
    ...
    const conv = await office.agent.post(`/api/offices/${office.id}/conversations`).send({ contactPhone: '+972501234567' }).then((r) => r.body);
    const res = await office.agent.post(`/api/offices/${office.id}/conversations/${conv.id}/inbound`).send({ text: 'יש לי תיק דיני משפחה' });
  ...
  it('closes and resumes a conversation', async () => {
    const office = await createOfficeWithSettings();
    const conv = await office.agent.post(`/api/offices/${office.id}/conversations`).send({ contactPhone: '+972501234567' }).then((r) => r.body);
    const closed = await office.agent.post(`/api/offices/${office.id}/conversations/${conv.id}/close`);
    expect(closed.body.status).toBe('closed');
    const resumed = await office.agent.post(`/api/offices/${office.id}/conversations/${conv.id}/resume-auto`);
